...
namespace ApiVela.Controllers
{
    ...
    public class VelaFraganciaController : ControllerBase
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> ActualizarVelaFragancia(Guid idVela, List<VelaFragancia> velaFragancia)
        {
            // Si tienes método para actualizar en repo, usarlo aquí, ejemplo:
            var resultado = await repo.ActualizarVelaFragancia(idVela, velaFragancia);
            if (resultado.Error != null)
                return BadRequest(resultado.Error.Mensaje);
